...
<template>
  <span class="stock-icon" :class="[`size-${size}`, `variant-${iconConfig.variant}`]" :style="iconStyle" aria-hidden="true">
    <svg viewBox="0 0 32 32" focusable="false">
      ...
      <template v-else-if="iconConfig.variant === 'leaf'">
        ...
        <path class="soft-stroke" d="M11 21c3.1-2.1 6.1-5.2 8.3-8.8" />
      </template>

      <template v-else-if="iconConfig.variant === 'capsule'">
        <rect x="6.5" y="10" width="19" height="12" rx="6" />
        <path class="soft-stroke" d="M16 10v12" />
      </template>
      ...
      <template v-else-if="iconConfig.variant === 'route'">
        <path class="soft-stroke" d="M7 23c4.7 0 4.2-8.2 9-8.2s4.4 8.2 9 8.2" />
      ...
      <template v-else-if="iconConfig.variant === 'play'">
        ...
        <path class="soft-stroke" d="M6.5 12.3c1.5-.7 2.4-.7 3.9 0M6.5 19.7c1.5-.7 2.4-.7 3.9 0" />
      </template>

      <template v-else-if="iconConfig.variant === 'wheel'">
        <circle cx="16" cy="16" r="7.8" />
        <circle class="cutout" cx="16" cy="16" r="3.1" />
        <path class="soft-stroke" d="M16 8.2v4.2M23.8 16h-4.2M16 23.8v-4.2M8.2 16h4.2" />
      </template>

      <template v-else-if="iconConfig.variant === 'chip'">
        <rect x="9.2" y="9.2" width="13.6" height="13.6" rx="2.2" />
        <rect class="cutout" x="13" y="13" width="6" height="6" rx="1.2" />
        <path class="soft-stroke" d="M12 7v3M16 7v3M20 7v3M12 22v3M16 22v3M20 22v3M7 12h3M7 16h3M7 20h3M22 12h3M22 16h3M22 20h3" />
      </template>

      <template v-else-if="iconConfig.variant === 'shield'">
        <path d="M16 6.5 24 9.3v6.4c0 4.4-3 8.3-8 10.1-5-1.8-8-5.7-8-10.1V9.3z" />
        <path class="soft-stroke" d="M16 10.8v9.1M12 15.3h8" />
      </template>
      ...
      <template v-else-if="iconConfig.variant === 'neural'">
        ...
        <circle class="cutout" cx="16" cy="16" r="3.1" />
        <path class="soft-stroke" d="M12 11.3 14.2 13M20 11.3 17.8 13M12 20.7 14.2 19M20 20.7 17.8 19" />
      </template>
      ...
      <template v-else-if="iconConfig.variant === 'pulse'">
        <path class="soft-stroke" d="M6.5 18.2h5l2.3-4.9 3.1 8.1 2.4-5.1h6.2" />
      ...
      <template v-else-if="iconConfig.variant === 'ticket'">
        ...
        <path class="soft-stroke" d="M16 10.6v10.8" />
      </template>

      <template v-else>
        <circle cx="16" cy="16" r="4.1" />
        <ellipse class="soft-stroke" cx="16" cy="16" rx="10" ry="4.8" />
        <ellipse class="soft-stroke" cx="16" cy="16" rx="4.8" ry="10" />
      </template>
    </svg>
  </span>
</template>
...
.stock-icon :deep(path),
.stock-icon :deep(rect),
.stock-icon :deep(circle),
.stock-icon :deep(ellipse) {
  fill: var(--icon-accent);
}

.stock-icon :deep(.soft-stroke) {
  fill: none;
